...
GLG의 1KB 정체성이 어느 하네스로 가도 같은 중력을 발휘하도록 — pi, Claude Code, Codex, Antigravity, Copilot, OpenClaw를 지나갈 때마다 — **정렬축을 보존하는 자리**다.
...
`./skills/` is the SSOT. `run.sh setup` symlinks them into pi, Claude Code, Codex, Antigravity, Copilot CLI (`~/.copilot/skills`), and the entwurf Claude plugin farm. (The Gemini CLI legacy surface was retired 2026-08-06 — the binary is gone. `~/.gemini/` still belongs to Antigravity.) See [README § What's Here](README.md#whats-here) for categories.
