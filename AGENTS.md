...
- **돌봄은 [sorge](https://github.com/junghan0611/sorge)의 자리다.** 리포를 가로지르는 시야와 "무엇을 이미 판정했는가"의 기억은 리포 하나가 가질 수 없다. sorge가 그 둘을 갖고, 담당자 문서 갱신·수선 스킬·리포 넘나드는 발견을 순회한다. 규칙 한 줄: **대신 해주지 않는다, 앞서 가서 담당자에게 그의 몫을 돌려준다**(vorausspringende Fürsorge). 담당자 문서 `20260227T031800`. 이 repo는 `skills/sorge` 심링크로 **연결만** 한다 — 실물 사본을 두지 않는다.
  - **자기 몫을 보는 손은 `board.py --mine` 이다** (2026-09-10). 인자 없이 부르면 cwd 의 git remote 로 집을 유추한다: `python3 ~/repos/gh/agent-config/skills/sorge/scripts/board.py --mine`. **이 집 안에서 `gh issue list` 만 쳐서는 자기 몫을 못 본다** — 실측으로 이 집 몫 17건 중 5건이 남의 리포(`sorge`·`andenken`)에 filed 돼 있었다. 상태는 이슈 라벨 세 축(`house:` · `state:` · `ball:`)에 살고 문서에 없다. `state`/`ball` 은 **단일값이라 add 가 아니라 set 이다** — 기존 값을 지우고 새 값 하나를 쓴다(`sorge/.claude/skills/sorge/scripts/labels.py § apply`).
  - **이 집은 `house:` 라벨을 갖는 두 집 중 하나다** (`sorge` 와 여기, `labels.py § COORD_HOUSES`). sorge 가 서기 전 여기가 coord 자리였던 유산 때문이다 — 남의 레인인데 여기 filed 된 이슈가 실재한다(`#13` → `house:forge-config`). **앞으로의 coord 이슈는 sorge 에 판다.** 이 라벨은 유산을 라우팅하는 자리이지 새 coord 를 여기 쌓으라는 허가가 아니다.
