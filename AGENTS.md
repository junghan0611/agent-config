...
- **돌봄은 [sorge](https://github.com/junghan0611/sorge)의 자리다.** 리포를 가로지르는 시야와 "무엇을 이미 판정했는가"의 기억은 리포 하나가 가질 수 없다. sorge가 그 둘을 갖고, 담당자 문서 갱신·수선 스킬·리포 넘나드는 발견을 순회한다. 규칙 한 줄: **대신 해주지 않는다, 앞서 가서 담당자에게 그의 몫을 돌려준다**(vorausspringende Fürsorge). 담당자 문서 `20260227T031800`. 이 repo는 `skills/sorge` 심링크로 **연결만** 한다 — 실물 사본을 두지 않는다.
  ...
  - **`house:` 는 덧셈이지 이사가 아니다** (실측 2026-09-10). `#13` 에 `house:forge-config` 만 붙여 「저 집 일이다」를 표현하려 했는데, 라벨에는 빼기가 없어 그 뜻은 적히지 않았다 — 이슈가 든 리포는 언제나 그 집 몫으로 남으므로 `--mine` 확정에 그대로 선다. **「내 몫 아님」을 적는 손은 아직 없다**(`gh issue transfer` 가 후보). GLG 판정 대기 — `NEXT.md` 의 「GLG 판정 대기」 목록에 있다(번호는 목록이 줄면 바뀌므로 항목 이름으로 찾아라).
...
- **새 하네스가 기본으로 켜는 내부 역할은 형제가 아니다.** reviewer·researcher·scout 같은 자동 역할이 사람의 요청 없이 호출 비용을 쓰게 두지 않는다. 발견하면 기본값에서 끄고, GLG가 그 역할을 명시해 요청한 때만 다시 판단한다. 이것은 내부 실행을 형제로 오독하지 않기 위한 바닥이다.
...
**Adding or removing a skill directory also edits `README.md`.** The § Skills paragraph carries a live count — *"N skills … the list below names M"* — and the category list names every skill individually. `N` = `git ls-files skills | cut -d/ -f2 | sort -u` minus `.gitkeep` — **tracked** directories, symlinked ones included. Do not use `ls -d skills/*/`: that glob also counts untracked machine-local symlinks (`butlercli` on the workstation today), and a number that counts what only one machine has is not a repo fact. `M` = every tracked skill the list names, and as of 2026-09-21 `M = N` — the holdout pair this rule used to name is gone (`voscli` dropped in `113e096` on 2026-05-18, `incidentcli` never tracked here at all). **A holdout is named individually or it does not exist; never carry a constant offset.** **Recompute; do not adjust the number by hand** — the count and the named list drift apart independently, and a number that still looks plausible is the failure mode. On 2026-09-09 that line went stale twice within hours: once when `dm` landed without being seated in the list, once when a sibling added and removed `skills/openclaw/` across two pushed commits (`362bcce`, `b18890a`). Neither author noticed; both were found by a later reader re-deriving the arithmetic.

That failure mode then produced its own textbook case, found 2026-09-18 by a reader re-deriving the arithmetic while writing `UNCLEBOB.md`. **This very rule said `N = M + 2` while also stating the glob already matches the symlinks — it double-counted them**, and anyone who followed it literally would have "corrected" a healthy 48 up to 50. Meanwhile README's *numbers* were both right (48 and 46) and its *composition* was wrong in two places that cancelled: the list named `butlercli`, which has never existed as a tracked directory in this repo, and omitted `subtract`, which landed in `6548399`. So verify the **named set**, not just the total: `python3 -c` the category groups against the tracked listing above (not `os.listdir('skills')` — 2026-09-21 measured that it returns 47 here against 46 tracked, because of the untracked `butlercli` link) and print both set differences. A matching count over a wrong set is the same defect this paragraph exists to catch. On 2026-09-21 the same paragraph was stale a third time in the other direction: the total said 48 against 46 tracked, and the two names it held out had both left the repo months earlier — which is why the offset is now forbidden outright.
