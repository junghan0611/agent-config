...
타임스탬프 아래 본문은 **인간과 다른 에이전트가 읽는 게시판 글**이다. 깔끔하게 적어야 한다.

### 형식 규칙

- **org-mode plain text** — 마크다운 표(`| col |`)가 아닌 org 리스트(`- `)를 쓸 것
...
- 관련 이슈: skshub-2ko (P0 bug)
```

```org
**** GPU 클러스터 vLLM 상태 점검 완료 :pi:sllm:gpu:
<2026-03-16 Mon 11:29>
- gpu1i/gpu2i: Qwen2.5-14B-AWQ 서빙중 (25일 가동)
- gpu3i: 튜닝전용 (유휴)
- 추론 테스트: 한국어 응답 정상
- 다음: Qwen3-0.6B 벤치마크 (IoT intent 파싱)
```

```org
**** agent-config: Phase 2 벤치마크 완료 :pi:commit:agentconfig:
<2026-03-15 Sun 13:22>
- 84,087 org chunks (768d Matryoshka)
- Hit 100% (19/19), MRR 0.872
- Jina rerank → MMR 교체 (역효과 발견)
- 비용: $0.06, DB: 752MB, 53 fragments
```
...
<2026-03-16 Mon 11:29>
```
→ 본문 없음. 무슨 작업인지 아무도 모름.

```org
**** 결과
<2026-03-16 Mon 11:29>
#+BEGIN_SRC json
{"status": "ok", "chunks": 84087}
#+END_SRC
```
→ 코드블록 불필요. org 리스트로 충분.
...
└────────┴──────────┴────────────┘
```
→ ASCII 테이블은 org-agenda에서 깨짐. org 리스트를 쓸 것.
