...
**핵심 함수 시그니처 — 본문 안 읽어도 이것만 보면 된다:**

```elisp
(agent-denote-add-history ID CONTENT)      ;; 히스토리 추가 (2인자)
(agent-denote-add-heading ID TITLE BODY)   ;; 헤딩 추가 (3인자)
(agent-denote-add-link ID TARGET-ID DESC)  ;; 링크 추가 (3인자)
```
...
## 2개의 소켓: server(에이전트) vs user(힣)

| 소켓 | 대상 | 용도 |
|------|------|------|
| `server` | 에이전트 daemon (nox) | `ec()` — org 조작, denote 함수, elisp 실행 |
| `user` | 힣의 GUI Emacs | `eu()` — 힣에게 파일 보여주기 |

```bash
# 에이전트 작업용 (기본)
ec() { emacsclient -s server --eval "$1"; }

# 힣에게 파일 보여주기 (현재 창에 열림, 블로킹 없음)
eu() { emacsclient -s user --no-wait "$@"; }
eu /path/to/file          # 힣의 현재 창에서 열기
eu -c /path/to/file        # 새 프레임으로 열기 (필요시)
```
...
> ```bash
> # ✅ 한 호출 안에서 정의 + 사용
> ec() { emacsclient -s server --eval "$1"; }; ec '(+ 1 1)'
>
...
```bash
# 매 bash 호출의 첫 줄에 넣는다
ec() { emacsclient -s /run/user/1000/emacs/server --eval "$1"; }
```

절대 경로로 소켓을 지정하면 GTK3/nox 관계없이 동작한다. nix store 해시를 하드코딩하지 않는다.

Docker에서는: `ec() { emacsclient -s /run/emacs/server --eval "$1"; }`
