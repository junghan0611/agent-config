...
(agent-denote-add-link ID TARGET-ID)       ;; 링크 추가 (2인자)
```
...
|------|------------------|------|
| Docker (OpenClaw 봇) | `/nix/store/.../bin/emacsclient` | `/run/emacs/server` |
| NixOS 로컬 (pi) | `emacsclient` (PATH) | `server` (기본값) |
