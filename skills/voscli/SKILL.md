...
## Anomaly EDN Contract (v0.4b, `anomaly` 출력 — agent 읽기용)

agent 가 작성하는 게 아니라 voscli 가 산출. **결과가 violation 으로 종료되면 EDN 이 없다 — 있는 것처럼 말하면 안 된다.**

v0.4a (daily-totals) + v0.4b (categorical) 가 한 EDN 안에 같이 박힌다. 한 명령 실행 = 한 산출물.

```clojure
{:anomaly/version       "v0.4b"               ; (구 v0.4a-2 / A1 — 2026-05-14 명명 정리)
 :anomaly/at            "2026-05-14T06:39:14Z"
 :anomaly/method        :modified-z-score
 :anomaly/rule          "modified-z = 0.6745 * (x - median) / MAD; flagged when |modified-z| >= z-threshold. MAD==0 또는 median==0 → modified-z nil → :anomaly/zero-baseline-spikes 분기 (delta >= spike-delta-threshold 일 때)."
 ...
 :anomaly/thresholds    {:z-threshold 3.5
                         :spike-delta-threshold 20}    ; v0.4b: zero/near-zero baseline spike 임계
 :anomaly/scopes-available [:full-day :operating-hours-only]
                                                       ; aggregate 가 ohs 분기 일관 박혀있으면 양쪽,
                                                       ; 아니면 [:full-day] 만. 일관성 깨지면 fast-fail

 ;; ─── v0.4a 호환 키 (full-day daily-totals) — top-level 유지 ───
 :anomaly/scope         :full-day
 ...
 [{:metric/key   :session-count
   ...
   :target       175 :z -0.347 :modified-z -0.638 :flagged? false}
  ;; 5 metrics: session-count / message-count / private-ratio / cs-sharing-ratio / messages-per-session
  ...]
 :anomaly/flagged-count 0

 ;; ─── v0.4b — full-day 의 by-product / by-topic / by-joint flagged ───
 :anomaly/by-product
 [{:anomaly/dimension :product
   :anomaly/key       "P_KT안심단말"          ; product/topic 은 string
   :anomaly/scope     :full-day
   :anomaly/baseline  {:n 4 :mean 8.0 :median 8.0 :mad 2.5 :min 5.0 :max 12.0
                       :values [12 5 8 7]}    ; baseline+target union zero-fill 적용 후
   :anomaly/target    91
   :anomaly/delta     83.0
   :anomaly/modified-z 22.39
   :anomaly/flagged?  true}
  ...flagged 만 박힘 (target 내림차순)]
 :anomaly/by-topic    [...]                     ; 동일 shape, key 는 string
 :anomaly/by-joint                              ; key 는 {:product P :topic T} map
 [{:anomaly/dimension :joint
   :anomaly/key       {:product "P_홈카메라프로" :topic "T_주문,배송,결제"}
   :anomaly/scope     :full-day
   :anomaly/baseline  {:n 4 :median 1.5 :mad 1.0 :values [1 0 1 2] ...}
   :anomaly/target    53 :anomaly/modified-z 34.74 :anomaly/flagged? true}
  ...]

 ;; ─── v0.4b — zero/near-zero baseline 의 spike (modified-z 정의 불가) ───
 :anomaly/zero-baseline-spikes
 [{:anomaly/dimension  :joint                   ; product / topic / joint 통합 — entry 별 dimension 명시
   :anomaly/key        {:product "P_KT안심단말" :topic "T_불량,고장(하드웨어)"}
   :anomaly/scope      :full-day
   :anomaly/baseline   {:n 4 :median 0 :mad 0 :values [0 0 0 0] ...}
   :anomaly/target     54
   :anomaly/delta      54
   :anomaly/spike-kind :zero-baseline}          ; baseline 모두 0
  {:anomaly/dimension  :topic
   :anomaly/key        "T_프로모션,행사,쿠폰"
   :anomaly/scope      :full-day
   :anomaly/baseline   {:n 4 :median 0 :mad 0 :values [0 0 0 0] ...}
   :anomaly/target     27 :anomaly/delta 27 :anomaly/spike-kind :zero-baseline}
  {:anomaly/dimension  :joint
   :anomaly/key        {:product "P_홈카메라프로" :topic "T_주문,배송,결제"}
   :anomaly/spike-kind :near-zero-mad           ; baseline 에 nonzero 1건은 있는데 median=0 또는 MAD=0
   ...}
  ...delta 내림차순 정렬]
 :anomaly/zero-baseline-spikes-count 9

 ;; ─── v0.4b — operating-hours-only 분기 (있을 때만) ───
 :anomaly/operating-hours-only
 {:scope                              :operating-hours-only
  :daily-totals                       [...]    ; same 5 metrics, ohs masked
  :flagged-count                      N
  :by-product                         [...]    ; entry 의 :anomaly/scope 도 :operating-hours-only
  :by-topic                           [...]
  :by-joint                           [...]
  :zero-baseline-spikes               [...]
  :zero-baseline-spikes-count         M}}
```
...
3. **MAD=0 → mod-z=nil → spike 분기로 자동 fallback**. zero-baseline-spikes 안에 박힘. 0 이라고 말하지 말 것 — `:spike-kind :zero-baseline` (baseline 모두 0) / `:near-zero-mad` (일부 nonzero 있지만 분포 폭 0) 둘 다 정직하게 표면화.
...
6. **v0.4b — 진원지는 by-* flagged + zero-baseline-spikes 두 분기를 함께 읽는다.** modified-z 가 정의되는 분포 (baseline 에 nonzero 가 충분) → flagged 분기. 정의 불가 (baseline 모두 0 또는 분포 폭 0) → spikes 분기. **둘 다 진원지로 동등하게 보고.** "P_홈카메라프로 × T_주문 mod-z=+34.74 (flagged) + P_KT안심단말 × T_불량 신규 54 (zero-baseline-spike)" 식으로 한 표면에 박을 것.
7. **scope 두 분기 (full-day vs operating-hours-only) 비교**: ohs-only 가 더 선명한 mod-z 를 내면 baseline 의 야간 노이즈가 효과를 묽혔다는 뜻. 같이 보고하면 baseline 정합성도 검증.
8. **`:anomaly/key` 는 evidence-lookup 으로 내려간다** — string 이면 `--by-product` / `--by-topic` 로, joint map (`{:product P :topic T}`) 이면 product 와 topic 둘 다 필터로 좁혀 chat-id + 본문 회수.
9. **scopes-available 가 `[:full-day]` 만** → aggregate 에 ohs 분기 부재. 운영팀이 ohs 결과 원하면 aggregate 부터 ohs 활성화로 다시 만들어야.
...
| `:scope-inconsistent` | aggregate 의 ohs 분기가 target/baseline 일부에만 있음 (v0.4b) | 2 |
...
### `compare day` (v0.3b) vs `anomaly` (v0.4a + v0.4b) — 언제 무엇을?

| 질문 | 적합한 명령 | 이유 |
|------|------------|------|
| "어제(5/3) 대비 오늘(5/4) 이 어떻게 다른가?" | `compare day --base-date 5/3 --new-date 5/4` | 두 날 직접 비교. ratio AND delta 룰. |
| "오늘(5/13) 이 평소와 다른가?" | `anomaly --target 5/13` | target 이전 분포 위 outlier. v0.4a daily-totals + v0.4b by-product/topic/joint + spikes 한 번에. baseline 자동/명시/window. |
| "어떤 product 가 폭증인가?" (총량 anomaly 의 진원지) | `anomaly` 결과의 `:anomaly/by-product` flagged + `:anomaly/zero-baseline-spikes` (dimension=:product) | v0.4b 박힘. 보조: `compare day --base-date X --new-date Y` 의 `:compare/notable-deltas` / `:newly-observed` 와 합산 가능 |
| "어떤 product × topic joint 가 진원지?" | `anomaly` 결과의 `:anomaly/by-joint` flagged + spikes (dimension=:joint, key={:product P :topic T}) | v0.4b. evidence-lookup 에 product + topic 둘 다 필터로 |
...
- **검출 방법 1차** = **modified z-score** (Iglewicz-Hoaglin, MAD 기반). 분위수 (percentile) / leave-one-out 은 v0.4c 후속.
- **z-threshold 기본값** = `3.5` (Iglewicz-Hoaglin 표준).
- **검출 metric (v0.4a 5개)** = `session-count` / `message-count` / `private-ratio` / `cs-sharing-ratio` / `messages-per-session` (파생). manager 응답 갭 metric 은 v0.4c.
...
### v0.4b 신규 (확정 — 2026-05-14, 구 v0.4a-2 / A1)

- **categorical 차원** = `:anomaly/by-product` / `:anomaly/by-topic` / `:anomaly/by-joint`. flagged modified-z entries 만 박힘 (target 내림차순).
- **key universe = baseline+target union** + **baseline 에 없는 key 는 0 으로 채움**. "어제 없던 제품이 오늘 54건" 같은 운영 신호를 잡으려면 zero-fill 이 정합.
- **zero/near-zero baseline spike** = baseline median=0 또는 MAD=0 → modified-z nil → `:anomaly/zero-baseline-spikes` 별도 분기 (`:spike-kind :zero-baseline | :near-zero-mad`). modified-z 로 억지 처리 안 함.
- **spike-delta-threshold 기본값** = `20` (v0.3b `:compare/anomalies` 의 `delta-threshold` 와 정합 — 운영팀 임계 통일).
- **scope** = aggregate 의 `:daily/operating-hours-only` 분기가 target+baseline 모두 일관 박혀 있으면 `:anomaly/operating-hours-only` 분기 자동 산출. **둘 다 항상**. 일관성 깨지면 `:scope-inconsistent` fast-fail.
- **scopes-available** = `[:full-day :operating-hours-only]` 또는 `[:full-day]` 만. 운영팀이 한 번에 양쪽 결과 본다.
- **entry meta** = 모든 by-* / spike entry 에 `:anomaly/scope` `:anomaly/dimension` `:anomaly/key` 보존 → evidence-lookup 으로 내려갈 수 있게.
- **명명 정리 (2026-05-14)** = 구 `v0.4a-2` / `A1` → `v0.4b categorical anomaly`. 구 `v0.4a-close` 는 별도 sub-phase 가 아니라 v0.4a 의 검증 이벤트로 흡수.

### v0.3 / v0.4 → 이후 이행

- **외부 신호 align (앱/펌웨어 릴리즈, Metabase, 휴일 baseline 매칭)** → v0.5 signal.
- **`compare week`** → 평일 baseline 분포 부족, 데이터 누적 후 합류.
- **분위수 / leave-one-out / manager 응답 갭** → v0.4c (구 v0.4b).
- **report 상단 anomaly 통합** → v0.4d (= v0.4 close, 구 v0.4c).

## 운영팀 에이전트 호출 패턴 (v0.4b)
...
|------------------|---------|---------|
| "오늘(5/13) 평소와 다른 점은?" | `anomaly --target 5/13` (한 번에 daily-totals + by-product/topic/joint + spikes + ohs) | daily-totals flagged 0 이어도 v0.4b by-* / spikes 가 진원지 박음. 비어있으면 `compare day` (5/12 → 5/13) 보조 |
| "어제(5/12) 대비 오늘(5/13) 변화?" | `compare day --base-date 5/12 --new-date 5/13` | notable-deltas / anomalies 둘 다 봄 |
| "어떤 product 가 폭증?" | `anomaly` 결과 `:anomaly/by-product` flagged + `:anomaly/zero-baseline-spikes` (dimension=:product) | `evidence-lookup --by-product <key> --keyword ...` 로 chat-id 회수 |
| "어떤 product × topic joint 가 진원지?" | `anomaly` 결과 `:anomaly/by-joint` + spikes (dimension=:joint, key={:product P :topic T}) | `evidence-lookup --by-product P --by-topic T --keyword ...` |
...
### v0.4 anomaly 결과 해석 룰

#### v0.4a — daily-totals (top-level)

1. **flagged-count 가 0** → "daily-totals 기준 평소 범위 안" 으로 솔직히 보고. **단, v0.4b by-* / spikes 까지 보고 종합 판단**. daily-totals 0 이라고 진원지 0 이 아니다 (5/13 smoke 의 핵심 발견 — daily-totals 0/5 인데 by-joint 9 flagged + spikes 9 박힘).
2. **modified-z 가 임계 미만이어도 의미** 있을 수 있다. 보조 신호로 (예: "mod-z +1.94 로 평소보다 높은 편") — 단 "anomaly 확정" 으로 말하지 말 것.
...
#### v0.4b — categorical (by-product / by-topic / by-joint + spikes + scope)

5. **진원지 = by-* flagged + zero-baseline-spikes 두 분기를 함께 본다.** 한 표면에 같이 박을 것 — modified-z 가 정의되는 분포 (baseline nonzero 충분) → flagged 분기, 정의 불가 (baseline 모두 0 또는 분포 폭 0) → spikes 분기. **운영팀이 봐야 할 진원지는 양쪽에 분산되어 있다.**
6. **`:spike-kind` 두 종류**: `:zero-baseline` (baseline 일자 모두 0 — "어제까지 없던 카테고리 신규 N건") / `:near-zero-mad` (baseline 에 nonzero 일부 있지만 분포 폭 0 — "거의 없던 카테고리가 갑자기 N건"). 둘 다 정직하게 표면화. 0 으로 말하지 말 것.
7. **scope 두 분기 (full-day vs operating-hours-only) 비교**: ohs-only 가 더 선명한 mod-z 를 내면 baseline 의 야간 노이즈가 효과를 묽혔다는 뜻. 같이 보고하면 baseline 정합성 검증.
8. **`:anomaly/key` 로 evidence-lookup**: string (`"P_홈카메라프로"`) → `--by-product` 또는 `--by-topic`. joint map (`{:product P :topic T}`) → 두 필터 모두 박아 chat-id + 본문 회수. ops/reviews 메모 작성 시 chat-id 1급 추적.

#### anomaly + compare day 합산 패턴

9. **mod-z 가 임계 미만 + spikes delta 임계 미만** 인데 운영팀이 "뭔가 평소와 다르다" 느끼면, `compare day` 의 `:compare/notable-deltas` (top-N) + `:compare/newly-observed` 를 병합해서 보고. anomaly 가 정직 표면화 못 하는 약한 신호를 compare day 가 보강.
10. **anomaly fast-fail 케이스**: 그날 baseline 이 n<3 이면 `:baseline-too-small`, 결과 EDN 없음. 이 경우엔 `compare day` 만으로 신호 표면화 가능 (baseline 1 일이라도 비교 가능). 단 "anomaly 결과처럼 말하지 말 것".
...
### 4회차 — v0.4a 운영팀 에이전트 smoke ✅ (2026-05-14)

Sonnet 분신이 voscli skill 만 들고 "5/13 평소와 다른 점이 있어?" 질문에 답함. 성공 기준 S1-S7 모두 통과, 실패 기준 F1-F6 회피.
