...
- [ ] **11. 판보기 — 이 집 몫 17건이 라벨로 섰다** ← NEW (2026-09-10). `sorge` 담당자가 넘긴 「판보기 버튼」을 받아 이 집 빚을 갚았다. **미분류 12 → 1** (남은 `sorge#17` 은 sorge 담당자가 찍는다 — 한 이슈의 생애를 두 손이 잡지 않는다). `state:ready` 넷이 실제 착수 대기다: **`sorge#6`**(omp 서브에이전트 규약) · **`sorge#7`**($TMUX_PANE 좌석 규약) · **`#16`**(`run.sh:1775` 한 줄) · **`#21`**(incubation recall). `ball:glg` 넷은 GLG 판정 대기 — `#1`·`#3`(제안) · `#17`(Upstage 계정 차단) · `#23`.
...
- **판보기 — 자기 몫은 자기 리포 안에서 안 보인다 (2026-09-10).**
  ...
  - **후보 판정이 휘발하던 자리는 닫혔다.** `andenken#13`·`#14` 를 내 몫으로 판정했는데 적을 라벨이 없어 sorge 에 돌려줬고, 그 집이 `d898a39` 로 고쳤다 — 원인은 내가 짚은 세 갈래 중 어느 것도 아니었고 게이트가 「어느 리포가 이슈를 드나」를 보고 있던 것이었다. 지금 이 집 판: **확정 19 · 후보 3**(실측 2026-09-10).
