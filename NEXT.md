...
- [ ] **11. 판보기 — 이 집 몫 17건이 라벨로 섰다** ← NEW (2026-09-10). `sorge` 담당자가 넘긴 「판보기 버튼」을 받아 이 집 빚을 갚았다. **미분류 12 → 1** (남은 `sorge#17` 은 sorge 담당자 소유). `state:ready` 넷이 실제 착수 대기다: **`sorge#6`**(omp 서브에이전트 규약) · **`sorge#7`**($TMUX_PANE 좌석 규약) · **`#16`**(`run.sh:1775` 한 줄) · **`#21`**(incubation recall). `ball:glg` 넷은 GLG 판정 대기 — `#1`·`#3`(제안) · `#17`(Upstage 계정 차단) · `#23`.
...
현재 좌표: **CURRENT 11(판보기 — 몫 17건이 라벨로 섰다)** — 10 의 「분류 한 판」이 라벨로 갈음됐다 · `state:ready` 넷이 착수 대기 · 3·4 보류(둘 다 남의 손 대기)

# NOW

- **판보기 — 자기 몫은 자기 리포 안에서 안 보인다 (2026-09-10).**
  ...
  - **아직 자리가 없는 것 하나:** `andenken#13`·`#14` 는 **내 몫이라고 판정했는데 그 판정을 적을 라벨이 없다.** `andenken` 은 coord house 가 아니라 `house:*` 정의를 안 갖는다. 후보 레인의 판정은 지금 아무 데도 안 남는다 — sorge 에 되돌려 물어야 한다.
  - **`state`/`ball` 은 단일값이라 set 이다.** `--add-label` 만 쓰면 재실행 때 값이 쌓이고 `board` 가 `⚠ 단일값 위반` 을 띄운다.
