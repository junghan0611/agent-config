...
* **herdr 이웃 장르를 처음으로 세었다.** awesome-herdr 2089개 행 중 cross-agent
  주소·메시지 장르는 약 20개이고 **17개 섹션에 흩어져 있다** — 그 목록에는 이 장르의
  칸이 없다. entwurf가 "Multi-agent fleets and supervisors"에 앉은 것은 잘못 신청한
  것이 아니라 taxonomy에 방이 없어서다. 이웃 13종을 README/PROTOCOL/SPEC 수준으로 읽고
  D0–D12로 판정했다: herdr 배달을 아예 안 쓰는 것은 `pi-peer` 하나뿐이고 그 대가가 pi
  단일 하네스이며, `herdr-swarm`은 화면 스크래핑을 완료의 권위로 채택했고(우리가 자격
  밖으로 명시한 그것), 우리 플러그인의 `[[build]]`+`[[panes]]` 조합은 전례가 0개다.
  (`a81df7f`, `6ee2848`)
* **그리고 그 문서가 자기 판정 하나를 뒤집었다.** 아침에 "패인보다 오래 사는 주소를 가진
  이웃 0건"이라 적었는데, 근거가 README 앞 35줄이었다. `kelpie`가 영속 logical agent와
  pane-bound incarnation을 분리하고 `adopt`로 재결속한다 — 정확한 문장은 "유일하다"가
  아니라 **"무동작 복귀는 아직 우리 쪽"**이다. F5를 다시 재고 F6–F8을 더했다. (`a81df7f`)
* **렌즈 5 — 검증면.** 하네스를 본다는 것은 그 테스트까지 본다는 것이다. coverage 퍼센트를
  묻지 않는 세 질문(행동 옆의 테스트 · 게이트 코드 자신의 검증 · 문서↔코드 계약 게이트)과
  대문자 문서 둘이 섰다 — `UNCLEBOB.md`(판정선을 의도적으로 넘고 헤더가 그렇게 말한다) ·
  `XIRP.md`(이 기계가 돌릴 수 없는 유일한 대상, macOS 전용). (`1e41475`)
* **오류가 어디서 들어왔는가를 별도 축으로 기록한다.** 무엇을 철회했는지(E)와 **어느 단계에서
  틀렸는지**(F)는 다른 정보이고, 다음 편집자에게 쓸모 있는 것은 후자다. 두 건 모두 압축된
  문장에 파일 경로와 타임스탬프가 함께 실려 있어서 복구됐다. (`5a7faaa`)

### 문서 대장

* **스킬 숫자 규칙이 바뀌었다 — 상수 오프셋 금지.** 같은 문단이 09-18에 이중계산으로, 오늘은
  반대 방향으로 틀렸다: 총계가 48인데 tracked 46이었고, 공개 목록 밖으로 이름을 부른 둘은
  **이미 몇 달 전에 리포를 떠났다**(`voscli` `113e096` 2026-05-18, `incidentcli`는 git
  히스토리 자체가 없다). 이름 목록은 정확했고 그래서 총계가 살아남았다. 그러니 숫자가 아니라
  규칙을 고쳤다 — `N`은 tracked 목록이고 `ls -d skills/*/`는 쓰지 않는다(기계 전용 untracked
  심링크를 세어 이 워크스테이션에서 47을 낸다). **홀드아웃은 개별로 이름을 부르거나 없다.**
  (`4146167`)
* herdr 벤치 행을 재측정했다 — `herdr --version` 0.9.1인데 표는 0.9.0이었고, 플러그인이
  marketplace 카드를 갖게 된 사실을 더했다. (`4146167`)

### Fixed

* **`~/.gitconfig`가 커밋 신원을 덮어쓰면 경고한다.** home-manager 뒤에 읽히므로 남아 있는
  `user.email`이 `vars.gitEmail`에서 귀속을 훔친다. (`ff64d9d`)
* **bibcli — X·YouTube 메타데이터가 무의미하면 거절한다.** 플랫폼 안내문을 abstract로 남기지
  않는다. (`638e0c3`)
* **pi `recall` 패키지를 agent 디렉토리 기준으로 resolve한다.** (`f5b7ee8`)

### Changed

* **quota에서 Copilot 레일을 내렸다** — 사용 중단이라 수집하지 않는다. 스케줄도 함께 리셋.
  (`aabf11a`)

### 늦게 옮긴 닫힘 — NEXT 좌표 둘

* **좌표 8 · 예상 답안 게이트 (#24)** — `v2026.9.9-gate.1`(2026-09-09)에서 GLG 지시로 닫혔다.
  담당자·검수자 권고는 "lint 미연결이라 아직 닫지 말 것"이었고 GLG가 그 위에서 정했다. 넘어온
  둘은 NEXT의 NOW에 계속 산다.
* **좌표 11 · 판보기 — 이 집 몫이 라벨로 섰다** (2026-09-10). 미분류 0으로 마감했고
  `state:ready` 넷과 `ball:glg` 넷이 그 뒤를 잇는다. 상태는 문서가 아니라 이슈 라벨에 산다.

  두 좌표 모두 닫힌 날짜가 이 태그보다 앞선다 — 그날 CHANGELOG로 옮기지 않아 NEXT의 RAIL에
  남아 있던 것을 여기서 갈무리한다.
...
다섯 건이 서로 다른 리포에서 같은 모양으로 나왔다 — 선언이 실행보다 작다고 말하거나,
크다고 말하거나. 갈림은 소리를 내지 않는다. 그래서 이 컷의 값은 고친 것보다 **다음에
소리가 나게 만든 것**에 있다.

### Changed

* **`forge` 스킬 실물을 `forge-config`로 넘기고 여기서는 연결만 한다** (`3b9f72e`). 이 repo의
  README가 자기 `forge` 항목을 스스로 반례로 지목하고 있었다 — *"thin pointer라 적혀 있는데
  361줄이 여기 산다"*. 이관의 근거는 취향이 아니라 로그였다: `bin/forge`를 만진 다섯 날짜가
  **전부** `SKILL.md`도 같이 만졌다. 한 변경에 두 리포 두 커밋. `forge-config` 담당자가 파일을
  byte-identical로 착지시키고(`dfe25c8`) 먼저 밀었고, 여기서 `git rm`+`ln`을 한 커밋에 담아
  이중 실물 구간을 닫았다. `sorge`·`voscli`·`incidentcli`에 이은 넷째다.

* **`LINKED_SKILL_NAMES` — 리포 이름과 스킬 이름이 처음으로 갈렸다** (`3b9f72e`).
  `LINKED_SKILL_REPOS`는 키를 리포명이자 스킬명으로 이중 사용했다. `sorge`는 둘이 같아 안
  드러났는데 `forge-config`가 `forge`를 소유하는 첫 사례라, 그대로 뒀으면 매 setup마다
  `skills/forge-config`를 찾다 못 찾고 *"link is dangling"* 거짓 경고를 냈다. 링크는 멀쩡한데
  경고만 뜨는 것이 제일 나쁜 종류의 거짓말이다.

* **`doctor:bins`가 조용한 실패를 이름으로 부른다** (`8ea458d`). provenance 대조는 이미
  있었다 — `env` 안에, ✅ 밑에. 배포가 건강한지 물을 때 사람이 치는 것은 `doctor:bins`이고,
  그것은 다섯 개 전부 초록을 냈다. 같은 순간 `env`는 `⚠ binary does not match the recorded
  build`를 내고 있었다. **검사가 없던 게 아니라 아무도 안 보는 자리에 있었다.** 이제 셋을
  가른다: `stale`(리포 소스가 배포본보다 최신 — 안 죽고 낡은 답을 준다) · `unprovenanced`(sha가
  `.provenance.json`과 다르다 — 안 죽고 우리가 가리킬 수 없는 코드다) · `arch`(다른 arch의
  기록). `wrong arch`는 원래대로 치명. 그리고 dictcli 독점이던 stale 검사를 Go CLI 넷에 돌렸다.

* **oracle이 첫 실행에서 두 건을 잡았다.** 그 기계는 aarch64에 원격이라 눈이 안 닿고,
  cross-compile을 안 하니 거기서만 참인 사실은 거기서만 검사된다. `denotecli(stale)`는
  예상했지만 **`bibcli(unprovenanced)`는 아무도 몰랐다** — 게이트 밖으로 얹힌 바이너리가
  거기 있었다.

* **`Pi ext / Pi skill: not linked` 오탐 제거** (`d34c720`). 요약이 `readlink`로 부모
  디렉터리 링크를 읽으려 했는데, semantic-memory는 확장에서 스킬로 옮겨갔고 `pi-skills/`는
  개별 링크 방식이 된 지 오래다. 읽을 링크가 없으니 "not linked"를 찍었다 — 확장 14개와
  스킬 46개가 링크돼 돌고 있는 기계에서. 세는 방식으로 바꿨다. **오라클 앞에 직접 서 봐서
  나왔지, 이 파일을 읽어서 나오지 않았다.**

### Added

* **`CLAUDE.md` 한 줄 — Claude Code가 이 집 `AGENTS.md`를 읽게 된다** (`ac35c02`).
  대조 실험(2026-09-04): 마커를 넣은 `AGENTS.md`만 둔 세션은 도구를 막으면 그 마커를 못 봤고,
  같은 마커를 `CLAUDE.md`에 넣으면 그대로 돌아왔다. 이 기계의 모든 세션 로그를 뒤지니 리포
  `AGENTS.md`가 주입된 사례는 **전부 그 리포에 이 한 줄이 있던 경우**였고, 45건 중 44건은
  전역 `home/AGENTS.md`였다 — 그게 배선이 있는 것처럼 보이게 만들었다. **오늘 하루 이 집에서
  일하면서 이 집 `AGENTS.md`를 한 번도 읽지 않았다.** 이미 16개 리포가 같은 줄을 갖고 있으니
  새 패턴이 아니라 누락이다. 같은 상태인 9월 활동 리포 13개는 횡단 발견으로 `sorge`에 넘겼다.

* **임베딩 유지보수의 자리를 `AGENTS.md`에 적었다** (`b835e2e`). `andenken-embed`는 andenken
  리포 안에만 있고, 나는 그것을 아침의 `forge`와 같은 누락으로 읽어 링크를 제안했다. 아니었다 —
  유료 임베딩과 파괴적 재구축을 라우팅하는 스킬이라 GLG가 **의도적으로** 전역에 두지 않았다:
  *"이 어려운 작업을 내가 아무나 불러서 하지 않거든."* 널리 보여야 하는 도구와 형태만 같고
  성질이 다르다. 그래서 스킬이 아니라 포인터를 둔다 — SSOT가 어디인지, 손이 둘뿐이라는 것
  (이 집 담당자 또는 andenken 담당자), 명령을 베끼기 전에 라우팅 표를 읽어야 한다는 것,
  전체 재구축 게이트는 에이전트가 못 넘는 대화형 `yes`라는 것.

### Docs

* **`emacs` 스킬이 감추고 있던 인자 형태** (`2510512`, `7554331`). `agent-denote-add-heading`은
  `(ID TITLE TAGS CONTENT AFTER-HEADING)`을 처음부터 받았는데 이 repo가 소유한 `SKILL.md`가
  그 형태를 안 적어놨고, 그래서 최소 네 집이 `:LLMLOG:` 헤딩을 raw elisp으로 손수 옮겼다.
  `description`이 "TAG 아니면 BODY"처럼 배타로 읽히던 자리도 고쳤다. 대소문자 감지 수선은
  doomemacs-config 커밋(`f752a07`)이 **설 때까지 미뤘다** — 미커밋 상태에서 적었으면 그 문장은
  두 시간 동안 거짓이었고, 거짓인 동안 아무 신호도 안 냈을 것이다.

* **`denotecli` — `date`는 생성 시각이지 수정 시각이 아니다** (`9825bbc`). `sorge`가
  `#+hugo_lastmod:`를 org 파일에서 직접 정규식으로 판 이유가 이 스킬의 JSON이 그것을 안 실어서고,
  그 과정에서 `HH:MM`을 잃어 같은 날 18:32 커밋을 21:55 도장보다 나중으로 셌다 — 없는 빚 1건.
  **문이 안 준 것은 사라지지 않고 남의 집에서 다시 태어난다.** 필드 표는 옮기지 않았다(옮기면
  그것도 혼자 늙는 사본이 된다) — 호출 자리에서 판단할 것만 싣고 전체 계약은 denotecli 리포와
  담당자 문서 id `20260222T090000`을 가리킨다.

* **README 스킬 수 정정** (`2510512`). `44 skills`라고 적혀 있었는데 실측 45였고 나열된 이름은
  43이었다(work 리포 링크 스킬 둘은 공개 목록에서 뺀다). 세 숫자가 서로 안 맞았다.
