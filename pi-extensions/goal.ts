 ...
 *
 * This is the piece pi's core deliberately leaves to extensions: an `agent_end`
 * handler may queue a message, and agent-session.ts then runs a continuation
 * turn for it (see the comment at core/agent-session.ts "queued by agent_end
 * extension handlers"). That is how a session wakes itself instead of parking
 * in `waiting for user`.
 *
 * Upstream: earendil-works/agent-stuff extensions/goal.ts (Armin/Mario).
...
export default function goalExtension(pi: ExtensionAPI) {
	...
	function queueContinuation(ctx: ExtensionContext): void {
		...
		try {
			...
			continuationQueued = false;
			const reason = `Failed to queue goal continuation: ${err instanceof Error ? err.message : String(err)}`;
			// Headless sessions (entwurf, -p) have no ui; notifying there throws and
			// swallows the real cause, so fall back to stderr.
			if (ctx.hasUI) ctx.ui.notify(reason, "error");
			else console.error(`[goal] ${reason}`);
		}
	}
