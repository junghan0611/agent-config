...
/**
 * GC stale sockets and dangling aliases.
 * A socket is stale if no process is listening on it.
 * An alias is dangling if its target socket file no longer exists.
 * Called once on session_start to keep the control directory clean.
 */
async function gcStaleSockets(): Promise<{ removedSockets: number; removedAliases: number }> {
	...
	try {
		...
		for (const entry of entries) {
			if (!entry.name.endsWith(SOCKET_SUFFIX) || entry.isSymbolicLink()) continue;
		...
		// Clean dangling aliases (target socket no longer exists)
		const afterEntries = await fs.readdir(CONTROL_DIR, { withFileTypes: true });
		...
		// GC is best-effort; don't crash the extension
	}
	return { removedSockets, removedAliases };
}
...
Messages automatically include sender session info for replies. When you want a response, instruct the target session to reply directly to the sender by calling send_to_session with the sender_info reference (do not poll get_message).`,
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			const sessionId = (params.sessionId ?? (params as Record<string, unknown>).targetSessionId as string | undefined)?.trim();
