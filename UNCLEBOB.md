...
부수적으로 25:40의 `take the code apart to get really tiny little functions`가 §A-4에서
...
### A-3. 균형 영수증 — "버리지 말고 tune하라". 어떤 건 풀고 **어떤 건 조인다**
...
이어지는 신입 교육론 (read at `50:41-51:00`). **이 구간의 위치를 내가 53–63분으로 잘못
적었고, 그 오류가 어디서 들어왔는지가 오늘 가장 날카로운 교훈이다 — 아래 §F를 보라.**
...
### A-4. 언어 표 — CRAP은 Clojure 전용이 아니고, TypeScript는 없다
...
### B. 세 코드베이스의 자기 모양 — 정규화되지 않은 대조이므로 순위표가 아니다
...
### C. entwurf 실측 — 첫 진단은 회수됐고, 남은 것은 미결정 둘이다
...
**그리고 같은 사람의 일반 입장 (read at `49:37`, §A-3):** *"어떤 건 restrictions를 풀고
...
- ~~**정본 63분 중 하네스 구간만 읽었고 53–63분 신입 교육론이 §D와 걸릴 수 있다.**~~
  **닫혔다.** 신입 교육론은 53–63분이 아니라 **49:37–51:00**이고(§F — 그 53–63이 어디서
  들어왔는지가 따로 기록할 값이 있다),
  terra 2차가 열어서 §A-3로 들어왔다 — **그리고 §D와 걸리는 정도가 아니라 문서 프레임을
...
*"same principles apply, different thresholds"*. §A-3가 그것이다. **내가 "미측정"으로 넘긴
...
### F. [2026-09-18] 오류가 들어온 자리 — 파일이 아니라 말이었다

이 문서는 대담 구간 하나의 위치를 **53–63분**으로 적었다가 **49:37–51:00**으로 정정했다.
처음에는 그것을 "다이제스트 요약 경유라서 생긴 오차"로 적었다. **그 귀속이 틀렸다.**

org 담당자가 2026-09-18에 자기 산출물을 확인하고 정정했다(그의 말):

> 저장된 다이제스트 **파일 자체는 이미 48:xx~53:xx로 정확히 잡혀 있었고**, "53–63분"으로
> 뭉뚱그린 건 GLG께 드린 **말로 된 요약** 쪽이었습니다 — 파일이 아니라 제 압축이
> 허술했던 겁니다.

**즉 산출물은 정확했고, 오류는 그것을 대화로 압축하는 단계에서 들어왔다. 그리고 나는 그
압축본 위에 결론을 세웠다.**

이것이 `~/AGENTS.md § How a fact crosses between siblings`의 한 줄이 가리키는 바로 그
실패다:

> **Brief a sibling off the sources, not off your summary of them.** A summary you wrote is
> where your own framing errors get laundered into someone else's premises.

오늘 이 문장의 실례가 둘 나왔고, 둘 다 같은 모양이다:

| 정확했던 원본 | 압축에서 깨진 것 | 그 위에 세워진 결론 |
|---|---|---|
| 다이제스트 파일 `48:xx~53:xx` | 대화 요약 "53–63분" | 나의 「미측정 구간」 판정 → 실제로 그 구간에 §A-3가 있었다 |
| 다이제스트 `"harness는 20배 토큰을 태운다"` (방향 정확) | 내 문서 "단일 에이전트가 20배 절약" | §D6의 인과 분해 전체 |

두 번째 줄은 내 쪽 압축이다. **받은 문장은 맞았고 내가 뒤집었다.**

> **교훈은 "요약을 믿지 말라"가 아니다.** 요약은 필요하고, 두 담당자 다 성실했다.
> 교훈은 **압축된 문장에는 원본으로 돌아가는 좌표를 같이 붙여야 한다**는 것이다.
> 파일 경로와 `mm:ss`가 함께 왔기 때문에 두 오류 모두 회수됐다. 좌표가 없었으면
> "53–63분"은 영구히 미측정 구간으로 남았을 것이고, §A-3는 이 문서에 없었을 것이다.

이 절이 §E(회수 목록)와 다른 이유: §E는 *무엇이* 회수됐는지의 목록이고, 이 절은 *오류가
어느 단계에서 들어왔는지*의 기록이다. 다음에 이 문서를 만지는 형제에게 후자가 더 쓸모 있다.
