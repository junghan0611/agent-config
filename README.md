...
**Code surface:** `forge` is the **code-side sibling of botment** — same single-bot identity (`glg-bot`), same footer-signature model, same closed-loop instinct, but pointed at a self-hosted Forgejo (`forge.junghanacs.com`, TBD) instead of remark42. v1 stub; implementation pending caddy + Forgejo deploy. Design lives in [`forge-config`](https://github.com/junghan0611/forge-config) (repo TBD) and is tracked in [agent-config #13](https://github.com/junghan0611/agent-config/issues/13).
