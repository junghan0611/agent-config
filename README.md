...
The same ownership split applies to Copilot CLI: `run.sh setup` links `~/.copilot/skills` (directory symlink to SSOT) and **keyset-merges** personal TUI prefs from `copilot/settings.fragment.json` into `~/.copilot/settings.json` (today: `mouse: false` — tmux owns scroll/wheel; harness must not capture mouse). The birth plugin marketplace unit and statusLine stay with entwurf (`install-copilot-bridge` / `install-copilot-statusline`). Do not symlink the whole Copilot settings file from this repo.

**Terminal control preference (GLG):** scroll/history belong to **tmux**, not the harness. Claude Code: `tui: "default"` plus env `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1` and `CLAUDE_CODE_DISABLE_MOUSE=1` in the settings fragments. Copilot: no alt-screen kill switch in 1.0.80 — `mouse: false` (or once `copilot --no-mouse`) is the available lever; internal keyboard viewport may still exist.
