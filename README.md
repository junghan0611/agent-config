...
**`permissions.defaultMode: "bypassPermissions"` — two surfaces, two owners, one value.** The native Claude Code config is agent-config's role, so both `settings.fragment.json` (workstation) and `settings.server.json` (server) pin the key here as of 2026-08-08. The ACP overlay is entwurf's role: `claude-agent-acp` reads settings through `CLAUDE_CONFIG_DIR`, so a peer session never sees `~/.claude/settings.json` at all and needs the same value pinned in [`overlay.ts`](https://github.com/junghan0611/entwurf) independently.
