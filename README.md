...
The same ownership split applies to Copilot CLI: `run.sh setup` links `~/.copilot/skills` (directory symlink to SSOT) and **keyset-merges** personal TUI prefs from `copilot/settings.fragment.json` into `~/.copilot/settings.json` (today: `mouse: true`). The birth plugin marketplace unit and statusLine stay with entwurf (`install-copilot-bridge` / `install-copilot-statusline`). Do not symlink the whole Copilot settings file from this repo.

**Terminal control preference (GLG):** scroll/history should belong to **tmux**, not the harness. Claude Code: `tui: "default"` plus env `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1` and `CLAUDE_CODE_DISABLE_MOUSE=1`. Copilot 1.0.80 always uses alt-screen (`github/copilot-cli#2148`, `#4509`; `--no-alt-screen` removed) — tmux scroll/copy-mode cannot own the transcript. Practical stopgap: keep `mouse: true` and use wheel / PgUp/PgDn / Home/End on the internal viewport; long copy via `/share` or `/exit print`.
